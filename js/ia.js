...
(function (g) {
  ...
  const CLASES_PILDORA = (function () {
    const clases = [
      ...
      { id: 'empujon', peso: 2, que: 'una verdad corta sobre el esfuerzo de hoy: lo que ' +
        'cuesta empezar, lo que pasa cuando uno aparece sin ganas, por qué el dia que ' +
        'menos apetece es el que mas cuenta' },
  ...
  function pildora() {
    ...
    /* El «3» es la versión de las instrucciones. Al cambiarlas, lo guardado con
    ...
    const clave = 'pildora3:' + hoyClave + ':' + bloque + ':' + clase.id +
    ...
    const prompt = contexto({ progreso: true, cargas: true, comida: true }) + '\n\n' +
      ...
      '- MÁXIMO 18 PALABRAS. Una frase. Si te sale una segunda, sobra.\n' +
      /* Lo que se colaba una y otra vez: «ya tienes seis entrenamientos esta
         semana; ve al gimnasio hoy». Eso no es un pensamiento, es el parte de
         estado que la pantalla ya da tres veces mas abajo, y encima con los
         numeros mal leidos. */
      '- NO LE CUENTES LO QUE HA HECHO. Nada de cuantos entrenamientos lleva, ni ' +
      'cuantos dias, ni si va bien o mal de racha: todo eso lo tiene en la misma ' +
      'pantalla, debajo de tu frase, y en cifras exactas. Tu frase es lo unico de ' +
      'esta pantalla que NO es un dato, y ese es su trabajo.\n' +
      '- Tampoco le mandes a entrenar ni le digas que descanse. No eres un aviso ni ' +
      'una tarea: para eso estan los botones.\n' +
      '- Lo que quiere leer ahi es un pensamiento corto y con fondo, de los que se ' +
      'quedan. Algo que le sirva hoy y que tambien sirviera dentro de un ano.\n' +
      '- Sus datos son para que aciertes con el tono —si lleva meses o acaba de ' +
      'empezar, si viene de una racha o de un parón—, no para que se los cuentes.\n' +
      /* Aunque ya no se le deja mandar a entrenar, esto se queda: sin decirselo,
         de vez en cuando escribia como si el dia estuviera por empezar. */
      (deHoy.length ? '- Hoy ya ha entrenado, hace un rato. No escribas como si el ' +
        'dia estuviera por empezar.' + SALTO : '') +
      '- Si lleva tiempo parado, que sea una invitación a volver, no un reproche; y ' +
      'sin nombrar el parón.\n' +
