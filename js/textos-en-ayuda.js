...
(function (g) {
  ...
  const AYUDA = {
    ...
    'Para algo que hiciste otro día —una caminata el domingo, la pachanga del sábado—, en Rutinas tienes *Apuntar algo que ya hice*. Ahí no hay series que apuntar, así que ese día sale en verde con los minutos debajo en vez del número de series; y la app sabe qué músculos mueve cada actividad, así que correr el domingo deja de contar como pierna abandonada el lunes. Si lo escribes con tus palabras y tienes la IA puesta, *Que lo mire la IA* afina las calorías y los músculos: ella distingue un partido de once de una pachanga, y el botón de esfuerzo no.':
      'For something you did on another day — a walk on Sunday, Saturday’s kickabout — Routines has *Log something I already did*. There are no sets to log there, so that day shows up green with the minutes underneath instead of a set count; and the app knows which muscles each activity moves, so running on Sunday stops counting as a neglected leg on Monday. If you write it in your own words and you have the AI set up, *Let the AI look at it* sharpens the calories and the muscles: it can tell a proper match from a kickabout, and the effort button cannot.',
