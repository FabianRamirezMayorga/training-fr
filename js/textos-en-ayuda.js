...
(function (g) {
  ...
  const AYUDA = {
    ...
    'Para algo que hiciste otro día —una caminata el domingo, la pachanga del sábado—, en Rutinas tienes *Apuntar algo que ya hice*. Ahí no hay series que apuntar, así que ese día sale en verde con los minutos debajo en vez del número de series; y la app sabe qué músculos mueve cada actividad, así que correr el domingo deja de contar como pierna abandonada el lunes. Si lo escribes con tus palabras, la app reconoce lo que pone y elige el esfuerzo sola: «Jugué fútbol» es un deporte de equipo y «subida al cerro» es senderismo. Si no reconoce nada se queda en «Otra cosa», que no apunta músculos porque puede ser cualquier cosa; ahí elige tú el botón que más se parezca. Y con la IA puesta, *Que lo mire la IA* lo afina más: ella distingue un partido de once de una pachanga, y el botón de esfuerzo no.':
      'For something you did on another day — a walk on Sunday, Saturday’s kickabout — Routines has *Log something I already did*. There are no sets to log there, so that day shows up green with the minutes underneath instead of a set count; and the app knows which muscles each activity moves, so running on Sunday stops counting as a neglected leg on Monday. If you write it in your own words, the app recognises what it says and picks the effort itself: “played football” is a team sport and “climbed the hill” is hiking. If it recognises nothing it stays on “Something else”, which logs no muscles because it could be anything; there you pick the closest button yourself. And with the AI set up, *Let the AI look at it* sharpens it further: it can tell a proper match from a kickabout, and the effort button cannot.',
